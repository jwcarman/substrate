...
@ConditionalOnProperty(
    prefix = "substrate.cassandra.atom",
    name = "enabled",
    havingValue = "true",
    matchIfMissing = true)
public class CassandraAtomAutoConfiguration {
  ...
  public CassandraAtomSpi cassandraAtom(CqlSession cqlSession, CassandraProperties properties) {
    CassandraAtomSpi atom =
        new CassandraAtomSpi(cqlSession, properties.atom().prefix(), properties.atom().tableName());

    if (properties.atom().autoCreateSchema()) {
      atom.createSchema();
    }
