...
@ConditionalOnProperty(
    prefix = "substrate.dynamodb.atom",
    name = "enabled",
    havingValue = "true",
    matchIfMissing = true)
public class DynamoDbAtomAutoConfiguration {
  ...
  public DynamoDbAtomSpi dynamoDbAtom(
      DynamoDbClient dynamoDbClient, DynamoDbProperties properties) {
    DynamoDbAtomSpi atom =
        new DynamoDbAtomSpi(
            dynamoDbClient, properties.atom().prefix(), properties.atom().tableName());

    if (properties.atom().autoCreateTable()) {
      atom.createTable();
    }
