...
class CassandraJournalPropertiesTest {
  ...
  @Autowired private CassandraJournalProperties properties;

  @Test
  void defaultsAreLoadedFromPropertiesFile() {
    assertThat(properties.prefix()).isEqualTo("substrate:journal:");
    assertThat(properties.tableName()).isEqualTo("substrate_journal");
    assertThat(properties.autoCreateSchema()).isTrue();
    assertThat(properties.defaultTtl()).isEqualTo(Duration.ofHours(24));
  }

  @EnableConfigurationProperties(CassandraJournalProperties.class)
  @PropertySource("classpath:substrate-journal-cassandra-defaults.properties")
  static class Config {}
}
