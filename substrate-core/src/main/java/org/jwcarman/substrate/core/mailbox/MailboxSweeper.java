...
public class MailboxSweeper implements AutoCloseable {

  private static final Log log = LogFactory.getLog(MailboxSweeper.class);
  static final int MAX_ITERATIONS_PER_TICK = 100;

  private final MailboxSpi spi;
  private final int batchSize;
  private final ScheduledExecutorService scheduler;

  public MailboxSweeper(MailboxSpi spi, Duration interval, int batchSize) {
    if (batchSize <= 0) {
    ...
    this.scheduler =
        Executors.newSingleThreadScheduledExecutor(
            Thread.ofVirtual().name("substrate-mailbox-sweeper", 0).factory());
  ...
  void tick() {
    try {
      ...
      if (totalDeleted > 0 && log.isDebugEnabled()) {
        log.debug("Swept " + totalDeleted + " expired mailboxes");
      }
    } catch (RuntimeException e) {
      log.warn("Mailbox sweep failed; will retry on next tick", e);
    }
  }
